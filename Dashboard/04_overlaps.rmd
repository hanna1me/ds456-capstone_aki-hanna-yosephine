...
stops           <- readRDS("../Dashboard/data/stops_gtfs.rds")
...
trip_routes <- sub_candidates %>%
  left_join(start_routes_lookup, by = c("start_stop_id" = "stop_id")) %>%
  left_join(end_routes_lookup,   by = c("end_stop_id"   = "stop_id")) %>%
  rowwise() %>%
  mutate(
    shared_routes = list({
      if (!is.null(routes_start) && !is.null(routes_end) &&
          !all(is.na(routes_start)) && !all(is.na(routes_end))) {
...
trip_routes <- trip_routes %>%
  mutate(num_shared_routes = lengths(shared_routes))

same_route_trips <- trip_routes %>%
  filter(num_shared_routes > 0)
```
...
same_route_trips_expanded <- same_route_trips %>%
  ...
  left_join(routes_gtfs %>% 
              select(route_id, route_short_name, route_long_name),by = "route_id")
```

```{r}
saveRDS(same_route_trips, "../Dashboard/data/same_route_trips.rds")
saveRDS(same_route_trips_expanded, "../Dashboard/data/same_route_trips_expanded.rds")
```
