...
trips_start <- valid_trips %>%
  left_join(
    road_centerlines %>% 
      select(GBSID, geometry), by = c("StartCenterlineID" = "GBSID")) %>%
  rename(start_geometry = geometry) %>%
  st_as_sf(crs = crs_proj)

trips_end <- valid_trips %>%
  left_join(road_centerlines %>% 
              select(GBSID, geometry),by = c("EndCenterlineID" = "GBSID")) %>%
...
boardings_sf <- boardings_csv %>%
  transmute(
    stop_id_raw   = as.character(Site_id),  
    boardings     = as.numeric(Ons),     
    alightings    = as.numeric(Offs),    
