...
avg_daily_overlap$Percent <- avg_daily_overlap$days_count / sum(avg_daily_overlap$days_count) * 100

ggplot(avg_daily_overlap, aes(x = "", y = 'days_count', fill = 'day_type')) +
  geom_col(width = 1) +
  geom_text(aes(label = paste0(round(Percent, 1), "%")),
            position = position_stack(vjust = 0.5), color = "white") +
  coord_polar(theta = "y") +
...
valid_trips %>%
  ...
  geom_col() +
  labs(
    title = "How long do people use Lime Scooter for",
    x = "Minutes",
    y = "Count"
  ) +
  scale_x_discrete(
...
valid_trips %>%
  ...
  mutate(DistanceCat = cut(
    TripDistance,
    breaks = seq(0, 3000, by = 100),   # 100-meter bins
    include.lowest = TRUE
  )) %>%
  count(DistanceCat) %>%   
  ...
  geom_col() +
  labs(
    title = "How far do people ride Lime Scooters?",
    x = "Meters (per 100)",
    y = "Count of trips"
  ) +
  scale_x_discrete(
    labels = seq(0, 2900, by = 100)
  ) +
  theme(
